...
For demonstration for general class anatomy, we here load an example 
`SpatialExperiment` (abbreviate hereafter with SPE) object (variable `spe`):
...
Noteworthily, the `colData`, `spatialData`, and `spatialCoords` slots follow
a hierarchical structure where `colData` > `spatialData` > `spatialCoords`. 
Here, each accessor allows joint accession of the target slot, and (optionally) 
any slot(s) that proceeds it.
...
`imgData` field as `DataFrame` of the following structure: 
...
of the `sample/image_id` arguments. Specifically, 
...
For example, `sample_id,image_id = TRUE,TRUE` will specify all images; 
`NULL,NULL` corresponds to the first image entry in the `imgData`; 
`TRUE,NULL` equals the first image for all samples; and `NULL,TRUE` 
...
The `SpatialExperiment` constructor provides with several arguments 
to aim at providing maximum flexibility to the user.
...
by specifying the columns' names that correspond to spatial coordinates and 
metadata, respectively, via `spatialCoordsNames` and `spatialDataNames`:
...
Alternatively, `spatialData/spatialCoords` may be supplied separately
directly as a `DataFrame/matrix`, e.g.:
...
Or, one of `spatialData/spatialCoords` can be supplied, while the other may be 
extracted from the input `colData` according to `spatialData/CoordsNames`: 
...
In general, `spatialCoords/DataNames` take precedence over `spatialCoords/Data`,
i.e., if both are supplied, the latter will be ignored. In other words,
`spatialCoords/Data` are preferentially extracted from the `DataFrame`s
...
Using `read10xVisium()`, the above code to construct the same SPE reduced to:
...
```{r}
sub <- spe3[, spe3$sample_id == "sample01.A"]
```

Or to retain only observations that map to tissue via:

```{r}
in_tissue <- spatialData(spe3)$in_tissue
sub <- spe3[, in_tissue]
sum(in_tissue) == ncol(sub)
```
