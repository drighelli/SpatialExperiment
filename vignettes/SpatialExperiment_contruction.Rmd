...
```{r}
spe3[, colData(spe)$sample_id=="section1.sample1"]
```

# Spot-based data (i.e. 10x Visium)

When working with spot-based data as 10x Visium or other technologies providing
...
Alternatively,  the `read10xVisium` function facilitates the import of 10x
Visium Data to handle one or more samples organized in
...
Finally, it is possible to construct a `SpatialExperiment` ovbject with two data 
slots: 
