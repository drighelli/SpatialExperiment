...
Both `spatialData` and `spatialCoords` are sperately stored inside the `int_colData` slot.
...
It is also possible to combine the `spatialData` and `spatialCoords` with the 
`colData` with two dedicated arguments.
