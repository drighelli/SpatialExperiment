...
via `spatialData(Names)` and `spatialCoords`, and storage of
images via `imgData`.
...
- `spatialDataNames`, a character vector specifying the `colData` subset containing  
spatial metadata (e.g. whether or not an observation was mapped to tissue)

`spatialCoords` are stored inside the `int_colData`, 
whereas `spatialDataNames` are stored inside the `int_metadata`.

`spatialCoords` are directly accessible via the corresponding accessor:

```{r}
head(spatialCoords(spe))
```

The `colData` subset specified by `spatialDataNames`
may be retrieved directly via the `spatialData` accessor:

```{r}
spatialData(spe)
identical(
  spatialData(spe),
  colData(spe)[spatialDataNames(spe)])
```
